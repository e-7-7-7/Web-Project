...
function getBooks(){
    ...
    if (userRole != 'Admin') {
        books = books.filter(book => book.isApproved);
    } 
...
function displayWantedBooks(searchValue){
    ...
    if(wantedBooks.length == 0){
        ...
        wantedBooks.map(b=>searchResults.appendChild(
            createBookCard(b.coverImageUrl,b.title,b.author,b.price,b.description,b.genre,b.quantity,b.sellerId,b.id, b.isApproved )))
    }
}
...
function approveBook(id) {
    ...
    if (bookIndex != -1) {
        ...
        displayWantedBooks(localStorage.searchValue); //  refresh customer view if admin is on the same page
    }
